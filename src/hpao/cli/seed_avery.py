...
from hpao.cli.seed import SCHOOL_NAME
...
async def seed_avery(db: AsyncSession) -> dict[str, object]:
    ...
    if school is None:
        raise RuntimeError(
            f"School {SCHOOL_NAME!r} not found. Run `python -m hpao.cli.seed` first."
        )
    ...
    if not classes:
        raise RuntimeError("No classes found. Run `python -m hpao.cli.seed` first.")
