import assert from "node:assert/strict";
import { readFile } from "node:fs/promises";
import test from "node:test";

const indexPath = new URL("../public/index.html", import.meta.url);
const appPath = new URL("../public/app.js", import.meta.url);
const stylesPath = new URL("../public/styles.css", import.meta.url);

test("UI includes a reset call control wired to restart the session", async () => {
  const [html, app, styles] = await Promise.all([
    readFile(indexPath, "utf8"),
    readFile(appPath, "utf8"),
    readFile(stylesPath, "utf8")
  ]);

  assert.match(html, /id="resetButton"/);
  assert.match(html, /Reset Call/);
  assert.match(html, /id="micToggleButton"/);
  assert.match(html, /id="callButtonLabel"/);
  assert.match(html, /Start Call/);
  assert.doesNotMatch(html, /Start Absentee Call/);
  assert.doesNotMatch(html, /id="hallPassButton"/);
  assert.doesNotMatch(html, /Start Hall Pass Call/);
  assert.doesNotMatch(html, /id="hallPassButtonLabel"/);
  assert.match(html, /id="caseStrip" hidden/);
  assert.doesNotMatch(html, /Loading attendance case/);
  assert.match(html, /Hold to Talk/);
  assert.doesNotMatch(html, /Enable Mic/);
  assert.doesNotMatch(html, /id="stopButton"/);
  assert.doesNotMatch(html, /role="tablist"/);
  assert.doesNotMatch(html, /id="callTab"/);
  assert.doesNotMatch(html, /id="systemTab"/);
  assert.doesNotMatch(html, /id="connectionStatus"/);
  assert.doesNotMatch(html, /class="eyebrow"/);
  assert.match(html, /<title>Attendence Officer Agent<\/title>/);
  assert.match(html, /<h1 id="app-title">Attendance Officer Agent<\/h1>/);
  assert.doesNotMatch(html, />Attendance Officer</);
  assert.doesNotMatch(html, />Call</);
  assert.doesNotMatch(html, />System</);
  assert.doesNotMatch(html, />Idle</);
  assert.match(html, /id="systemView"/);
  assert.doesNotMatch(html, /id="submissionTab"/);
  assert.doesNotMatch(html, /id="submissionView"/);
  assert.doesNotMatch(html, /Submission Info/);
  assert.doesNotMatch(html, /contenteditable="true"/);
  assert.doesNotMatch(html, /data-editable-key/);
  assert.doesNotMatch(html, /README quick start/);
  assert.doesNotMatch(html, /README datasets/);
  assert.doesNotMatch(html, /YOUR VIDEO MUST BE RECORDED WITH LOOM!/);
  assert.doesNotMatch(html, /Screen capture:/);
  assert.doesNotMatch(html, /Other track options/);
  assert.match(html, /System architecture/);
  assert.match(html, /Use case processes/);
  assert.match(html, /Use Case: Teacher Records Attendance/);
  assert.match(html, /Use Case: Student Uses Hall Pass/);
  assert.match(html, /Use Case: ABE Evaluates Risk And Policy/);
  assert.match(html, /Use Case: Voice Agent Calls Guardian/);
  assert.match(html, /<svg class="architecture-diagram"/);
  assert.match(html, /<svg class="process-diagram"/);
  assert.match(html, /id="themeToggle"/);
  assert.match(html, />ABE</);
  assert.match(html, />Data</);
  assert.match(html, />Logic</);
  assert.match(html, />Policy</);
  assert.doesNotMatch(html, /Attendence Bot \+ E/);
  assert.doesNotMatch(html, /Data: students, guardians, events/);
  assert.doesNotMatch(html, /Logic: trends, queues, thresholds/);
  assert.doesNotMatch(html, /Policy: district guidance and LLM/);
  assert.doesNotMatch(html, /Attendance entry/);
  assert.doesNotMatch(html, /history review/);
  assert.doesNotMatch(html, /Bilingual guardian call/);
  assert.doesNotMatch(html, /excuse capture/);
  assert.doesNotMatch(html, /attendance event/);
  assert.doesNotMatch(html, /hall pass event/);
  assert.doesNotMatch(html, /guardian call task/);
  assert.doesNotMatch(html, /excuse result/);
  assert.doesNotMatch(html, /updated status/);
  assert.match(html, /View Dashboard/);
  assert.match(html, /Update Status/);
  assert.match(html, /Start Hall Pass/);
  assert.match(html, /Return Hall Pass/);
  assert.match(html, /Load Students/);
  assert.match(html, /Call Guardian/);
  assert.match(html, /Record Conversation/);
  assert.doesNotMatch(html, /class="step-copy"/);
  assert.doesNotMatch(html, /Core Data Store/);
  assert.doesNotMatch(html, /Core Logic and Policy/);
  assert.match(html, /href="\.\/styles\.css"/);
  assert.match(html, /src="\.\/app\.js"/);
  assert.match(app, /restartSession/);
  assert.match(app, /resetButton\.addEventListener/);
  assert.match(app, /startPushToTalk/);
  assert.match(app, /stopPushToTalk/);
  assert.match(app, /handlePushToTalkKeyDown/);
  assert.match(app, /micToggleButton\.addEventListener\("pointerdown", startPushToTalk\)/);
  assert.match(app, /micToggleButton\.addEventListener\("pointerup", stopPushToTalk\)/);
  assert.match(app, /micToggleButton\.addEventListener\("pointercancel", stopPushToTalk\)/);
  assert.match(app, /micToggleButton\.addEventListener\("pointerleave", stopPushToTalk\)/);
  assert.match(app, /micToggleButton\.addEventListener\("keydown", handlePushToTalkKeyDown\)/);
  assert.match(app, /micToggleButton\.addEventListener\("keyup", handlePushToTalkKeyUp\)/);
  assert.match(app, /setCallButtonStarted/);
  assert.match(app, /isCaseSummaryVisible/);
  assert.match(app, /hideCaseSummary/);
  assert.match(app, /End Call/);
  assert.match(app, /callScenarios/);
  assert.match(app, /caseSummary/);
  assert.match(app, /absentee call/);
  assert.match(app, /hall pass call/);
  assert.match(app, /replaceChildren\(title, detail\)/);
  assert.match(app, /was absent today and has had 14 hall passes in the last 10 school days/);
  assert.match(app, /valid reason for the absence and hall passes/);
  assert.match(app, /has had 14 hall passes in the last 10 school days for a total of 4 hours absent/);
  assert.match(app, /Do you want to share a reason for those hall passes, or should I record that no excuse was provided/);
  assert.match(app, /one speaking turn and wait for the guardian response/);
  assert.match(app, /14 hall passes in 10 school days \| 4 hours absent/);
  assert.match(app, /Start Call/);
  assert.doesNotMatch(app, /Start Absentee Call/);
  assert.match(app, /Do not substitute a different attendance issue/);
  assert.match(app, /use it only as a tie-breaker for ambiguous one-word replies/);
  assert.match(app, /If the guardian says "yes", continue in English/);
  assert.match(app, /If the guardian says "sí" or "si", continue in Spanish/);
  assert.match(app, /Would you prefer English or Spanish\? ¿Prefiere inglés o español\?/);
  assert.doesNotMatch(app, /Data Saved/);
  assert.doesNotMatch(app, /data\/output\/conversations\.csv/);
  assert.doesNotMatch(app, /Realtime session created/);
  assert.doesNotMatch(app, /Excuse saved as/);
  assert.doesNotMatch(app, /hallPassButton/);
  assert.match(app, /setMicrophoneListening\(false\)/);
  assert.match(app, /Release to Send/);
  assert.match(app, /Hold the talk button while the parent speaks/);
  assert.match(app, /Mic paused/);
  assert.match(app, /transcriptLabelFor/);
  assert.match(app, /conversation\.item\.input_audio_transcription\.completed/);
  assert.match(app, /assistant: "Ava"/);
  assert.match(app, /parent: "GUARDIAN"/);
  assert.doesNotMatch(app, /stopButton/);
  assert.doesNotMatch(app, /activateTab/);
  assert.doesNotMatch(app, /systemTab/);
  assert.doesNotMatch(app, /connectionStatus/);
  assert.doesNotMatch(app, /submissionTab/);
  assert.doesNotMatch(app, /submissionView/);
  assert.doesNotMatch(app, /setupEditableSubmissionFields/);
  assert.doesNotMatch(app, /attendanceOfficerSubmission:/);
  assert.match(app, /toggleTheme/);
  assert.match(app, /attendanceOfficerTheme/);
  assert.match(app, /Hello this is Ava, your school attendence agent\. Is this/);
  assert.match(app, /apiPath/);
  assert.match(app, /http:\/\/localhost:5178/);
  assert.match(app, /session\?scenario=/);
  assert.match(app, /startListenOnlySession/);
  assert.match(app, /recvonly/);
  assert.match(app, /Microphone Blocked/);
  assert.match(styles, /\.controls\s*{[^}]*align-items: center;/s);
  assert.match(styles, /\.controls\s*{[^}]*align-content: flex-start;/s);
  assert.match(styles, /\.controls button\s*{[^}]*height: 42px;/s);
  assert.match(styles, /\.controls button\s*{[^}]*max-height: 42px;/s);
  assert.match(styles, /\.controls button\s*{[^}]*white-space: nowrap;/s);
  assert.doesNotMatch(styles, /\\.tab-button/);
  assert.doesNotMatch(styles, /status-pill/);
  assert.doesNotMatch(styles, /\\.eyebrow/);
});
